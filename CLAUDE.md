...
**Content is data-driven.** Each page defines plain arrays of objects at the top of the file (`highlights` and `socials` in `page.tsx`; `experiences` and `projects` in `experience/page.tsx`) and `.map()`s over them. To add or edit content, modify the array entry — do not hand-write repeated JSX blocks.

**Org logos** live in `public/logos/` with snake_case filenames (e.g. `cutc_logo.jpg`, `yale_logo.png`, `hack404_logo.jpg`). Two components render them, both in `components/`:
